...
%%%% the real domain (Two frequencies)
...
%%%% Last modified: March 3, 2024
   ...
   %% Time Domain Parameters:
    Fs1 = .5;                   % Original signal frequency 1 [Hz]
    Fs2 = 3;                     % Original signal frequency 2 [Hz]
    StopTime = 4.5;              % Length of the sample [s]
    tstep=.01;                   % Time resolution [s]
    t = (0:tstep:StopTime)';     % Time vector [s]
   
   %% Cosine  Wave Generation:
    %x = (((cos(2*pi*Fs1*t)))+((cos(2*pi*Fs2*t)))+2)/2; % Centered
    x = ((cos(2*pi*Fs1*t))+(cos(2*pi*Fs2*t)));        % Non-centered
   ...
   for i=1:nfreq  
       ...
       %x = ((cos(Fs1/Fs_w(i)*t2))+(cos(Fs2/Fs_w(i)*t2))+2)/2;      % centered case
       x = (cos(Fs1/Fs_w(i)*t2))+(cos(Fs2/Fs_w(i)*t2));             % non-centered case
       ...
       if (Fs1/Fs_w(i) ==1 || Fs2/Fs_w(i)==1)
           ...
           text(450,3.2,['Fs_w=',num2str(Fs_w(i)),' Hz'],'Color','red','FontSize',14);
           pause();
           clf(pp);
       else 
           at1=nexttile(1,[2 1]);
